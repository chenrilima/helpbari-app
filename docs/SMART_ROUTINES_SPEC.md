...
### 4.7.1 Pipeline determinístico do Occurrence Engine

O pipeline puro é `OccurrenceBlueprint → ScheduleInstantResolver → UUIDv5 →
OccurrenceWindowResolver → RoutineOccurrence`. Regras wall-clock resolvem
`LocalDate + TimeOfDayValue` exclusivamente no timezone IANA persistido no
schedule; timezone do dispositivo e relógio global não participam.

No avanço de DST, um horário inexistente é movido ao primeiro minuto local
válido posterior e marcado `shiftedForward`. No retorno, um horário ambíguo gera
uma única obrigação, usando a primeira ocorrência cronológica e marcando
`ambiguousEarlierOffset`. Políticas nominais de rejeição existem para ambos os
casos. O resultado preserva horário solicitado, horário efetivo, offset e
diagnóstico.

A identidade usa UUIDv5, namespace exclusivo
`f45b7f48-8f4d-5c67-9d7e-6bc0f7f8345b` e schema `v1`. O nome UTF-8 canônico é:

```text
v1|routine:<uuid>|plan:<uuid>|schedule:<uuid>|date:<yyyy-MM-dd>|time:<HH:mm:00>|tz:<iana>|seq:<inteiro>
```

Ela não inclui `displayOrder`, instante resolvido, offset, status ou alvo
reagendado. Assim, reagendar mantém o ID e `originalScheduledFor`; apenas
`currentWindow.scheduledFor` muda.

Cada schedule carrega uma `OccurrenceWindowDefinition` explícita. Os instantes
UTC materializados são `windowStartsAt = target - earlyTolerance`,
`onTimeEndsAt = target + onTimeTolerance` e `windowEndsAt = min(target +
lateTolerance, próximo slot distinto)`. A janela operacional é semiaberta
`[windowStartsAt, windowEndsAt)` e não possui tolerância global escondida.

`EveryNHoursRule` possui `anchorAtUtc` obrigatório. Seus slots são
`anchorAtUtc + n × intervalHours` no intervalo UTC semiaberto solicitado, com
limite explícito. A cadência absoluta não se desloca em DST; somente sua
representação `LocalDate/TimeOfDayValue` no timezone clínico pode mudar. Isso é
intencionalmente distinto das regras wall-clock. Permanecem adiados os efeitos
do Adherence Engine, conflitos de eventos, persistência, sync e projeções de
notificação.
